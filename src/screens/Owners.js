        ...
        <TableBody>
          {users.map((row) => (
            ...
            >
              ...
              <TableCell><button type="button" className={classes.btn} onClick={()=>{navigate('/OwnerOperation',{state:{row}})}}>Edit</button><button className={classes.btn1} type="button" onClick={()=>( fetch(`http://evspoint.com/api/user/${row.contactNo}`,{
     method: 'DELETE'
   }).then((result) => {
